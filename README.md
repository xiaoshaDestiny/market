...
| **分层** | **职责** | **建设要领** |
| --- | --- | --- |
| **Adapter 适配层** | 适配产品建设层面所需要的接口
无线端、PC端、App端等 | 适配前端展示、路由适配 |
| **Application 应用层** | 获取输入、构建上下文、参数校验
二方RPC服务调用入口
消息发送出口、消息接受入口
定时任务调度入口 | 可直接绕过领域层，访问基础设施层，应用内不是全部功能模块都适合做成领域模型 |
| **Domain 领域服务层** | 封装领域服务，对上层应用提供服务 | 保障核心代码不会受到外部干扰
Domain依赖基础设施的实现 |
| **Infrastructure  基础设施层** | CRUD、搜索引擎、文件系统、分布式服务的RPC | 需要依赖Domain提供的接口 |
...
梳理清楚之后，结合COLA的应用架构分层思想，划分 GoodCase 和 BadCase
:::info
GOOD CASE

复杂功能：
前端适配层接入 - 应用层解析校验参数 - 领域模型依赖自身资源（Infrastructure ）和 外部依赖（Facade）对请求处理 - 处理时若有异常则透出

简单功能：
前端适配层接入 - 应用层解析参数 - 直接访问对应基础设施或者二方服务处理完毕 - 有异常则处理异常

二方应用依赖自身应用提供心服务：
应用层面直接新增接口 - 观察判断现有领域是否满足
若满足：直接将领域服务结果进行解析，包装成标准的返回结果返回，版本升级即可联调
若不满足：判断是否需要新增模型、或者对现有模型进行改造、或者直接使用某基础设施即可完成

自身服务某个功能需要依赖其他二方服务：
找到对应的依赖之后，封装参数，对Client进行调用，对结果转换为自身领域所需要的结构，根据业务判断发生异常是否阻断还是不阻断
:::
:::danger
BAD

张冠李戴型：做了不在职责范围内的工作
在 Controller 中，直接访问 Mapper 完成CRUD

复杂功能，前端入参、DB访问入参、查询返回值是同一个对象。出现了向上向下多个层次的对象，DO、VO、Query 等。

在核心业务代码的Service中，构建参数、查询其他服务、结果解析、异常处理，完全在同一个方法中。

这些Case会导致的问题：
1、结构的跨越度大，带来代码整洁度差、理解成本高
2、稳定性难以保障，单测也很难执行
3、代码可复用程度底，或基本不可复用

总体来说就是代码会逐渐腐坏，越来越难维护
:::
...
| 领域层，领域层是否可以直接使用基础设施的实现？严格规范来说，不可以。
如果不可访问，导致Manager和Mapper是必须实现的，代码冗余度会加剧。考虑再三，决定将该限制放开，在领域层可以直接使用Mapper | 未严格限制，根据需要开放可选
严格规范： Domain - Manager - Mapper
不限制：可直接 Domain -Mapper |
...
| **Infrastructure**
**Facade** | 基础设施和防腐结果在层级上是一致的，原因是：都是为领域模型提供支撑，支持的来源可以是二方服务（防腐），也可以是自身可维护的DB、Oss、ES、Redis等（基础设施）
![image.png](https://cdn.nlark.com/yuque/0/2024/png/2355110/1709707730731-ed8a50ee-fff6-43e0-be05-0cc56ac76e8e.png#averageHue=%231f2124&clientId=ufb11f149-e9ee-4&from=paste&height=451&id=u5d6f5729&originHeight=901&originWidth=819&originalType=binary&ratio=1&rotation=0&showTitle=false&size=62959&status=done&style=none&taskId=uabb087ad-b235-4d88-82c1-90f4fc3760f&title=&width=410) | 可见范围：
上层不可见，基础设施不可见防腐的原始结果，防腐不可见Mappper
...
| 模块名称 | 依赖项目内 | 依赖COLA |
| --- | --- | --- |
| market-adapter | market-application 
market-component-common 
market-component-client  | 
 |
| market-application | market-component-common
market-component-client
market-domain-core
market-domain-adapter | 
 |
| market-component-client | 无  该包给二方用，RPC参数 | cola-component-exception
cola-component-dto |
| market-component-common | 无  该包给二方用，前端请求结构 | cola-component-exception
cola-component-dto |
| market-domain-adapter | 无  该包定义了Domian 依赖什么样的接口完成自己的核心工作 | cola-component-exception
cola-component-domain-starter |
| market-domain-core | market-domain-adapter
market-facade
